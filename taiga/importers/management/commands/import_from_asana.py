...
from taiga.importers.asana import AsanaImporter
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if options.get('token', None):
            ...
            url = AsanaImporter.get_auth_url(settings.ASANA_CLIENT_ID, settings.ASANA_CLIENT_SECRET, "https://tree.taiga.io/importers/asana")
            print("Go to here and come with your code (in the redirected url): {}".format(url))
            code = input("Code: ")
            access_data = AsanaImporter.get_access_token(code, settings.ASANA_CLIENT_ID, settings.ASANA_CLIENT_SECRET, "https://tree.taiga.io/importers/asana")
            token = access_data
